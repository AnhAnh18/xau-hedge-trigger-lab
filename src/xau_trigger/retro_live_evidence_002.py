...
def verify_evidence_aggregate(value: Mapping[str, Any], *, expected_input_digest: str | None = None, expected_component_digest: str | None = None) -> bool:
    ...
    gates = load_gate_registry()["gates"]
    totals = value["totals"]
    if totals["buy_actions"] + totals["sell_actions"] > totals["action_count"] or totals["action_count"] > totals["observed_actions"] or totals["state_matches"] > totals["state_checkpoints"] or totals["state_checkpoints"] > totals["observed_actions"] or totals["direction_matches"] > totals["direction_comparable"] or totals["direction_comparable"] > totals["action_count"] or totals["order_matches"] > totals["order_comparable"] or totals["order_comparable"] > totals["action_count"] or totals["timing_matches"] > totals["timing_comparable"] or totals["timing_comparable"] > totals["observed_actions"] or totals["eligible_checkpoints"] > totals["observed_actions"] or totals["lot_matches"] > totals["lot_comparable"] or totals["lot_comparable"] != value["cycle_count"]:
        raise RetroBotInputError("E-002 aggregate totals invariant invalid")
    if totals["duplicate_actions"] > totals["observed_actions"] or totals["censored_checkpoints"] + totals["comparable_checkpoints"] != totals["eligible_checkpoints"] or totals["robustness_passes"] > totals["robustness_cases"]:
        raise RetroBotInputError("E-002 aggregate denominator invariant invalid")
